...
function useImportMapOverridesSingleton() {
  ...
  const overrides = useLocalStorage<IOverrideInfo[]>(IMPORT_MAP_MANAGER_OVERRIDES_KEY, [])
  ...
  const processOverrides = computed(() => {
    ...
    const result = Object.keys(imports).map((moduleName) => {
      return {
        ...
        isOverride: overrides.value.some(({ name, enabled }) => name === moduleName && enabled)
      }
    })
    ...
    return [...result, ...overridesParsed].sort((a, b) => +b.isOverride - +a.isOverride)
  })
  ...
  const { insertOverrideMap, getDefaultMap,fetchExternalMap } = useOverridesUtils()
  ...
  onMounted(async () => {
    ...
    if (externalImportMap.value?.enabled) {
      ...
      overridesFromImportMap.value = { ...overridesFromImportMap.value, ...external }
    }
  })
  ...
  watch(
    ...
    (val) => {
      ...
      const imports = onlyEnables.reduce((acc: Record<string, string>, { name, url }) => {
        acc[name] = url
        return acc
      }, {})
  ...
  return { processOverrides, externalImportMap, overrides, overridesFromImportMap, getItemOverride }
}

export const useImportMapOverrides = createSharedComposable(useImportMapOverridesSingleton)
